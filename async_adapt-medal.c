...
const static char* newRound="newround";
 ...
 */
static uint16_t random_int(uint16_t size_num){
...
/*---------------------------------------------------------------------------*/
/**
 * @brief neighbors_length
 * @return
 */
static uint8_t neighbors_length(){
    uint8_t k, neigh_count= 0;
...
uint8_t neighbors_1hop(){
    ...
    for(k = 0; k < num_neighbors; k++){
        node_info_t *nptr = &neigh_table[k];
        if ((nptr->node_id != 0) && (nptr->hopcount == 1)){
            neigh_count++;
        }
    }
    return neigh_count;
...
static char aloha_transmit(void){
    ...
    dpacket.node_id =  (rimeaddr_node_addr.u8[0]  & 0xff);
...
static void packet_input(void){
    ...
    if(pkt->ptype == DATA_PACKET){
        ...
        if(all_neighbors_found_flag == 0){
            ...
            ///curr_num_neighbors
            ///
            uint8_t tmp_num_neighs = neighbors_length();
    ...
    while(1) {
        ...
        if(command != NULL){
            ...
            if(!strcmp(command, "newround")){
                ...
                callback_time = RTIMER_NOW() + TS*(1+0/*random_int (100)*/);
                ref_timer = callback_time;
...
PROCESS_THREAD(nodeoutput_process, ev, data){
    ...
    while(1){
        ...
        /** PRINTF(">:%d,%d,%u,%u,%u,%u\n", rounds_counter, node_id,
                               CHANNEL_POOL_SIZE, curr_num_neighbors,
                               neighbors_1hop(), discovery_time);*/
	
	//NETSTACK_MAC.input();
	
	if(get_curr_neighbors() == num_neighbors){
	      PRINTF("END>:%d,%d,%u,%u,%u,%u\n", get_elapsed_rounds(), rimeaddr_node_addr.u8[0],
                  CHANNEL_POOL_SIZE, get_curr_neighbors(), neighbors_1hop(),
